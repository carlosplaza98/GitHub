...
class NoticiaController
{
    ...
    public function index(){
        ...
        $rowset = $this->db->query("SELECT * FROM news ORDER BY fecha DESC");
    ...
    public function activar($id){
        ...
        if ($noticia->activo == 1){
            ...
            $consulta = $this->db->exec("UPDATE news SET activo=0 WHERE id='$id'");
            ...
            //Activo la noticia
            $consulta = $this->db->exec("UPDATE news SET activo=1 WHERE id='$id'");
        ...
        if (isset($_POST["guardar"])){
            ...
            if ($id == "nuevo"){
                ...
                if ($imagen){
                    if (is_uploaded_file($imagen_recibida['tmp_name']) && move_uploaded_file($imagen_recibida['tmp_name'], $imagen_subida)){
                        $texto_img = " La imagen se ha subido correctamente.";
                        $this->db->exec("UPDATE news SET imagen='$imagen' WHERE id='$id'");
                    }
                    else{
                        $texto_img = " Hubo un problema al subir la imagen.";
                    }
